...
df = df[df['country'] == 'america'].iloc[:, 1:]
    ...
    if candidate_cloned != 'none' and candidate_cloned != None and candidate_cloned != "NaN":   
        ...
        try:
            for method in original_winners[file]:
                baseline_winner = original_winners[file][method].strip("{}").strip("'").replace("''", "").replace('""', '"').replace('/', "").replace(" (UNKNOWN)", "").replace("' ", "").replace(" '", "'")
                new_winner = row[method].strip("{}").replace(" (UNKNOWN)", "").strip("'").replace("''", "").replace('""', '"').replace('/', "").replace("' ", "").replace(" '", "'")
                # print(original_winners[file][method], baseline_winner)
                # print(row[method], new_winner)
