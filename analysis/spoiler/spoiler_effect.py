...
file_path = '/Users/belle/Desktop/build/rcv_proposal/analysis/spoiler/australian_results.csv'  # Replace with your actual file path
...
for _, row in df.iterrows():
    ...
    if candidate_removed != 'none':  
        ...
        for method in original_winners[file]:
            baseline_winner = original_winners[file][method].strip("{}").strip("'")
            new_winner = row[method].strip("{}").strip("'")
            ...
            if normalize(baseline_winner) == normalize(candidate_removed.strip("{}").strip("'")) or row[method] == "unknown":
                continue
